...
- [Make the most of your Google Analytics data with Orchest and Meltano](https://github.com/astrojuanlu/orchest-google-analytics) -<!--o--> Export the raw events generated by Google Analytics 4 to your data warehouse, using Orchest for orchestration, Meltano for Extraction & Loading (EL), and Metabase for visualization <!--o-->- `elt` `pipeline` `meltano` `google-analytics`
- [Detect anomalies on your time series data with Orchest and Clarify](https://github.com/astrojuanlu/orchest-timeseries-clarify) -<!--o--> Create a pipeline that loads time series data from Clarify, trains an anomaly detection model, writes back the anomalies, and notifies you <!--o-->- `pipeline` `clarify` `time-series` `anomaly-detection`
- [Drift report with Evidently](https://github.com/ricklamers/orchest-hello-evidently) -<!--o--> Create a drift report using Evidently <!--o-->- `drift` `evidently`
- [Analyzing +4.6M Reddit comments with DuckDB](https://github.com/astrojuanlu/orchest-duckdb) -<!--o--> Analyze +4.6M Reddit comments with DuckDB from Parquet files <!--o-->- `duckdb` `sql` `arrow`
